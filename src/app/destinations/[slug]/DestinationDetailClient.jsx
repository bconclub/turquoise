...
export default function DestinationDetailClient({ initialDestinationData, slug }) {
  const router = useRouter();
  const [destinationData, setDestinationData] = useState(initialDestinationData);
  const [loading, setLoading] = useState(!initialDestinationData);

  useEffect(() => {
    if (slug && !initialDestinationData) {
      loadDestination();
    }
  }, [slug, initialDestinationData]);
                    ...
                    >
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
                          ...
                          {pkg.starting_price && !isNaN(Number(pkg.starting_price)) && (
                            <div className="flex items-center gap-1 font-semibold text-turquoise-600">
                              <DollarSign className="w-4 h-4" />
                              <span>
                                {pkg.currency || 'INR'} {Number(pkg.starting_price).toLocaleString()}
                              </span>
                            </div>
                          )}
                        </div>
